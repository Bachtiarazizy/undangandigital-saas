  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Get session to check user role (if needed for role-based routing)
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const session = await getSession();

        // You can use session data for role-based routing if needed
        // For example: if (session?.user?.role === 'admin') router.push('/admin-dashboard');
      ...
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>Masuk untuk mengelola undangan digital Anda</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="password" type={showPassword ? "text" : "password"} placeholder="Masukkan password" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-10 pr-10" required />
